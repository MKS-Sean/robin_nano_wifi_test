...
/*
static void esp_dma_pre() {
  DMA_HandleTypeDef *DMArx = &wifiUsartDMArx;
  CBI32(wifiUsartDMArx.Instance->CR, 0);
  wifiUsartDMArx.Instance->M0AR = (uint32_t)WIFISERIAL.wifiRxBuf;
  wifiUsartDMArx.Instance->NDTR = 0x0000;
  wifiUsartDMArx.Instance->NDTR = UART_RX_BUFFER_SIZE;
  __HAL_DMA_CLEAR_FLAG(DMArx, DMA_IT_TC | DMA_IT_HT | DMA_IT_TE | DMA_IT_FE | DMA_IT_DME);
  SBI32(wifiUsartDMArx.Instance->CR, 0);
}
*/
#ifdef __cplusplus
...
{
  #if 1
  wifiUsartDMArx.Instance->CR &= ~( 1 << 0 ) ;

	wifiUsartDMArx.Instance->NDTR = UART_RX_BUFFER_SIZE;
  wifiUsartDMArx.Instance->PAR = (uint32_t)&(USART1->DR);
    ...
    wifiUsartDMArx.Init.Priority = DMA_PRIORITY_VERY_HIGH;
    #ifdef STM32F4xx
    ...
    if (HAL_DMA_Start_IT(&wifiUsartDMArx, (uint32_t)&(USART1->DR), (uint32_t)WIFISERIAL.wifiRxBuf, UART_RX_BUFFER_SIZE)) {
      Error_Handler();
    }
...
{	
  if(__HAL_DMA_GET_FLAG((DMA_HandleTypeDef *)&wifiUsartDMArx, DMA_FLAG_TEIF2_6) != RESET) {
    /* Disable the transfer error interrupt */
    __HAL_DMA_DISABLE_IT((DMA_HandleTypeDef *)&wifiUsartDMArx, DMA_IT_TE);

    /* Clear the transfer error flag */
    __HAL_DMA_CLEAR_FLAG((DMA_HandleTypeDef *)&wifiUsartDMArx, DMA_FLAG_TEIF2_6);

    /* Update error code */
    SET_BIT(wifiUsartDMArx.ErrorCode, HAL_DMA_ERROR_TE);

    /* Change the DMA state */
    wifiUsartDMArx.State = HAL_DMA_STATE_ERROR;

    /* Process Unlocked */
    __HAL_UNLOCK(&wifiUsartDMArx); 

    if (wifiUsartDMArx.XferErrorCallback != NULL) {
      /* Transfer error callback */
      wifiUsartDMArx.XferErrorCallback((DMA_HandleTypeDef *)&wifiUsartDMArx);
    }
  }  
	if (__HAL_DMA_GET_FLAG((DMA_HandleTypeDef *)&wifiUsartDMArx, DMA_FLAG_HTIF2_6) != RESET) {
		__HAL_DMA_CLEAR_FLAG((DMA_HandleTypeDef *)&wifiUsartDMArx, DMA_FLAG_HTIF2_6);
		WIFI_IO1_SET();
	}
		
	if (__HAL_DMA_GET_FLAG((DMA_HandleTypeDef *)&wifiUsartDMArx, DMA_FLAG_TCIF2_6)!= RESET) {
		__HAL_DMA_CLEAR_FLAG((DMA_HandleTypeDef *)&wifiUsartDMArx, DMA_FLAG_TCIF2_6);
		
		if(esp_state == TRANSFER_IDLE) {
			esp_state = TRANSFERING;
		}
		
		if(storeRcvData((uint8_t *)WIFISERIAL.wifiRxBuf, UART_RX_BUFFER_SIZE)) {
			esp_dma_pre();
